...
void Lemon::ErrorMsg(const char *, int,const char *, ...);
...
struct lemon *lemp;
{
  ...
  if( lemp->start ){
    ...
    if( sp==0 ){
      Lemon::ErrorMsg(lemp->filename,0,
...
struct lemon *lemp;
{
  ...
  ** start nonterminal.  */
  ...
  for(rp=lemp->rule; rp; rp=rp->next){
    ...
    Lemon::ErrorMsg(lemp->filename,rp->ruleline,"This rule can not be reduced.\n");
    lemp->errorcnt++;
  }
}
  ...
  for(cfp=current; cfp; cfp=cfp->next){
    ...
    if( sp->type==NONTERMINAL ){
      if( sp->rule==0 && sp!=lemp->errsym ){
        Lemon::ErrorMsg(lemp->filename,rp->line,"Nonterminal \"%s\" has no rules.",
          sp->name);
        lemp->errorcnt++;
      }
...
void Lemon::ErrorMsg(const char *filename, int lineno, const char *format, ...){
  ...
  switch( psp->state ){
    ...
    case WAITING_FOR_DECL_OR_RULE:
      if( x[0]=='%' ){
        ...
        if( psp->prevrule==0 ){
          Lemon::ErrorMsg(psp->filename,psp->tokenlineno,
...
struct lemon *gp;
{
  ...
  if( fp==0 ){
    Lemon::ErrorMsg(ps.filename,0,"Can't open this file for reading.");
    gp->errorcnt++;
    return;
  }
  fseek(fp,0,2);
  ...
  if( filebuf==0 ){
    Lemon::ErrorMsg(ps.filename,0,"Can't allocate %d of memory to hold this file.",
      filesize+1);
    gp->errorcnt++;
    return;
  }
  if( fread(filebuf,1,filesize,fp)!=filesize ){
    Lemon::ErrorMsg(ps.filename,0,"Can't read in all %d bytes of this file.",
      filesize);
    free(filebuf);
    gp->errorcnt++;
    return;
  ...
  for(cp=filebuf; (c= *cp)!=0; ){
    ...
    if( c=='\"' ){                     /* String literals */
      ...
      if( c==0 ){
        Lemon::ErrorMsg(ps.filename,startline,
"String starting on this line is not terminated before the end of the file.");
      ...
      if( c==0 ){
        Lemon::ErrorMsg(ps.filename,ps.tokenlineno,
...
PRIVATE void translate_code(struct lemon *lemp, struct rule *rp){
  ...
  if( rp->lhsalias && !lhsused ){
    Lemon::ErrorMsg(lemp->filename,rp->ruleline,
